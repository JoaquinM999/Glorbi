 ...
 *  All business logic and UI are unchanged.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard eyebrow="Net Worth" value="$25,430.50" badge="Margin Balance" badgeType="neutral" />
        <StatCard eyebrow="Net PNL" value={fmtUsd(stats.netPnl)} badge={`${filteredData.length}d · PNL + Funding`} badgeType={stats.netPnl >= 0 ? 'positive' : 'negative'} />
        <StatCard eyebrow="Floating PNL" value={fmtUsd(345.20)} badge="▲ En posiciones abiertas" badgeType="positive" />
        <StatCard eyebrow="Profit Factor" value={`${stats.profitFactor.toFixed(2)}x`} badge={`Win Rate ${stats.winRate.toFixed(1)}%`} badgeType={stats.winRate >= 50 ? 'positive' : 'negative'} />
      </div>
      ...
      <AIExecutiveSummary stats={stats} fgValue={fgData?.value ?? null} btcDom={glbData?.btcDom ?? null} balance="25,430.50" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4 flex items-center justify-center">
          <WinRateDonut winRate={stats.winRate} winDays={stats.winningDays} lossDays={stats.losingDays} beDays={stats.breakevenDays} />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="relative w-40 h-40">
      <svg viewBox="0 0 120 120" className="w-full h-full -rotate-90">
        ...
        <circle cx="60" cy="60" r="48" fill="none" stroke="#22C55E" strokeWidth="14" strokeDasharray={`${winDash} ${C}`} strokeDashoffset="0" strokeLinecap="butt" />
        <circle cx="60" cy="60" r="48" fill="none" stroke="#EF4444" strokeWidth="14" strokeDasharray={`${lossDash} ${C}`} strokeDashoffset={`${-winDash}`} strokeLinecap="butt" />
      </svg>
